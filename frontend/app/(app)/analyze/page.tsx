...
export default function Home() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!text.trim()) {
        setExtractError(
          'Не удалось извлечь текст из документа — похоже, это скан (изображение) без текстового слоя, ' +
          'а не текстовый файл. Распознайте текст любым OCR-инструментом (например, Яндекс.Документы, ' +
          'Google Диск при открытии скана, или встроенное распознавание в Adobe Acrobat) и загрузите ' +
          'результат — или пересохраните документ из исходного текстового источника.'
        );
        return;
      }
